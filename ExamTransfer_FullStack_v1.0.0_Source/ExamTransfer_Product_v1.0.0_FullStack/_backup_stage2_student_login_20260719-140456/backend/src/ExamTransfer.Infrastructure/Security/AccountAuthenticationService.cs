    ...
    IOptions<ExamTransferOptions> options) : IAccountAuthenticationService
{
    ...
    public async Task<AccountLoginResultDto> LoginAsync(AccountLoginRequest request, string? ipAddress, CancellationToken cancellationToken)
    {
        ...
        {
            if (string.IsNullOrWhiteSpace(user.StudentCode) || string.IsNullOrWhiteSpace(user.DisplayName))
            {
                throw new ApiException(
                    ErrorCodes.StudentIdentityMismatch,
                    "Hồ sơ sinh viên chưa có đủ mã sinh viên và họ tên.",
                    422);
            }

            var challenge = challenges.IssueChallenge(
                user.Id,
                request.DeviceId.Trim(),
                request.MachineName.Trim(),
                TimeSpan.FromMinutes(Math.Clamp(appOptions.Auth.ChallengeMinutes, 1, 15)));
