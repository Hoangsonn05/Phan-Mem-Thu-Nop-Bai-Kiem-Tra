    ...
    IOptions<ExamTransferOptions> options) : IAccountAuthenticationService
{
    ...
    public async Task<PasswordChangeResultDto> ChangePasswordAsync(
        AccountTokenPrincipal principal,
        ChangePasswordRequest request,
        CancellationToken cancellationToken)
    {
        var validation = await sessions.ValidateAsync(principal, cancellationToken)
            ?? throw new ApiException(
                ErrorCodes.LoginSessionExpired,
        ...
        if (user.Role != UserRole.Student)
        {
            throw new ApiException(
                ...
                "Chức năng đổi mật khẩu bắt buộc hiện chỉ dành cho tài khoản sinh viên.",
                403);
        }

        ValidatePasswordChangeRequest(user, request);

        if (identityProvider is not IExternalAccountSecurityService accountSecurity)
        {
            throw new ApiException(
                ErrorCodes.PasswordChangeFailed,
                "Nhà cung cấp đăng nhập hiện tại không hỗ trợ đổi mật khẩu.",
                503);
        }
        ...
        if (string.IsNullOrWhiteSpace(user.SupabaseAuthUserId))
        {
            throw new ApiException(
                ErrorCodes.PasswordChangeFailed,
                "Tài khoản chưa được liên kết đầy đủ với Supabase Auth.",
                409);
        }

        var account = FirstNonBlank(user.Email, user.StudentCode, user.Username)
            ?? throw new ApiException(
                ErrorCodes.PasswordChangeFailed,
                "Không xác định được tài khoản Supabase cần đổi mật khẩu.",
                409);

        await accountSecurity.ChangePasswordAsync(
            new ExternalPasswordChangeRequest(
        ...
        // sessions.ValidateAsync returns an AsNoTracking user. Reload the local
        // entity as a tracked entity before updating the SQLite profile flag.
        var trackedUser = await db.UsersSet
            .FirstOrDefaultAsync(x => x.Id == user.Id, cancellationToken)
            ?? throw new ApiException(
                ErrorCodes.PasswordChangeFailed,
                "Khong tim thay ho so cuc bo de hoan tat viec doi mat khau.",
                409);
        ...
        return new PasswordChangeResultDto(
            true,
            false,
            "Đổi mật khẩu thành công. Tài khoản đã sẵn sàng sử dụng.");
    }
    ...
    private static void ValidatePasswordChangeRequest(User user, ChangePasswordRequest request)
    {
        ...
        if (normalizedPassword.Contains("uneti", StringComparison.Ordinal)
            || (!string.IsNullOrWhiteSpace(user.StudentCode)
                && normalizedPassword.Contains(user.StudentCode.Trim().ToLowerInvariant(), StringComparison.Ordinal))
            || (!string.IsNullOrWhiteSpace(user.Username)
                && user.Username.Length >= 5
                && normalizedPassword.Contains(user.Username.Trim().ToLowerInvariant(), StringComparison.Ordinal)))
        {
            throw new ApiException(
                ErrorCodes.PasswordPolicyRejected,
