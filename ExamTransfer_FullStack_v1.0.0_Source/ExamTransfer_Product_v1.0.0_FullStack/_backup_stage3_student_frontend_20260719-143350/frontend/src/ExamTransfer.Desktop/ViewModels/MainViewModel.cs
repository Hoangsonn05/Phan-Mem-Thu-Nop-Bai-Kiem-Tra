...
public sealed class MainViewModel : ObservableObject
{
    ...
    public string PageTitle => Selected?.Title ?? (authState.IsAuthenticated ? "ExamTransfer" : "Đăng nhập");
    public string PageDescription => Selected?.Description ?? (authState.IsAuthenticated ? "Thu và gửi bài thi an toàn trong mạng LAN" : "Sử dụng tài khoản được cấp sẵn");
    public string PageGlyph => Selected?.Glyph ?? "\uE8A5";
